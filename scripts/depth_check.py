...
saying what is in it, a command that would put a card on file, a part carrying one of the paper keys
ADR-0006 removed, or a hub that quietly went back to teaching.
...
# ADR-0006 removed papers from this curriculum: no `papers/` folder, no paper document, and no
# citation anywhere. These three keys were that format's whole surface in a part's frontmatter, so
# rejecting them is what stops the removed contract coming back a day at a time.
FORBIDDEN_FRONTMATTER_KEYS = ["papers", "paper", "kind"]
...
def check_sections(content: str, where: str, report: Report) -> None:
    ...
    if meta is None:
        ...
        for key in FORBIDDEN_FRONTMATTER_KEYS:
            if key in meta:
                report.fail(
                    where,
                    f"frontmatter carries {key}: - this curriculum cites no papers, so there is no "
                    f"`papers:`, `paper:` or `kind: paper` key to carry (§17.4.2, ADR-0006)",
                )

    content = body(text)
    check_sections(content, where, report)
...
def check_day(number: int) -> Report:
    ...
    if (folder / "papers").is_dir():
        report.fail(
            "papers/",
            "this curriculum has no papers/ folder - ADR-0006 removed it, and what a paper "
            "document taught belongs in the part that uses the idea (§17.4.2)",
        )
