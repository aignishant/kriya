...
**Where the doubling comes from.** Not from convention, and not from taste. In the late 1980s the
early internet collapsed: links that carried thirty-two kilobits a second fell to about forty bits a
second, and they did it without anything breaking. Senders that gave up waiting and sent again
immediately were the whole cause. The fix that came out of that collapse, and that everything on the
internet has used since, is to **double the wait after every failure**. Senders who cannot see each
other and cannot agree on anything can still back off together if each one halves its own rate, and
no gentler shape gets a jammed network out of the jam. Jitter is a separate idea, added later,
and the rest of this part is about why doubling alone is not enough without it.
